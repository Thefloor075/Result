...
size_train = size(features_train);
size_test = size(features_test);
...
for i_run=1:n_runs
    ...
    for t=2:t_inputs
        ...
        C = coef_a*res_out .* inv_255 + (1-coef_a)*C;
        % record reservoir history
        res_history(1:size_res,t) = C;
...
%         end
    ...
    write_result(a,1) = err_train;
    write_result(a,2) = err_test;
    write_result(a,3) = coef_a;
    write_result(a,4) = res_scores(i_run);
    a = a+1;
...
filename = strcat( num2str(size_res),'_rapport','_linear_memory','.csv');
column_head = {'TrErr','TrErr','coef_a','Score'};
